...
export async function POST(req: NextRequest) {
  try {
    const { name, email, reason, message, phone } = (await req.json()) as {
      name?: string;
      email?: string;
      reason?: string;
      message?: string;
    ...
    await redis.set(
      ticketId,
      { name, email, reason: reason || "Something else", message: message || null, phone: phone || null, createdAt: new Date().toISOString() },
      { ex: 60 * 60 * 24 * 365 }
    );
