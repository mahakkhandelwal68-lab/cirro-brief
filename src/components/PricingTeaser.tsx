...
export function PricingTeaser() {
  ...
  const plans = [
    ...
    { key: "annual", icon: <CrownIcon />, name: "Annual", note: "For regular publishing with the best value.", value: pricing ? `From ${fmt(pricing.symbol, pricing.annual)}/year` : "—", iconColor: "var(--orange)", iconBg: "var(--orange-tint)", cta: "View Pricing", badge: "Best Value" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
          {plans.map((p, i) => (
            <Reveal key={p.key} delay={i * 80}>
              <div className="hover-pop" style={{ position: "relative", background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: "26px 22px", display: "flex", flexDirection: "column", gap: 10, minHeight: 210 }}>
                {p.badge && (
                ...
                <div className="icon-badge" style={{ background: p.iconBg, color: p.iconColor }}>{p.icon}</div>
                <span style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 18 }}>{p.name}</span>
                <span style={{ fontSize: 14, color: "var(--text2)", flex: 1 }}>{p.note}</span>
                <span style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 15, color: "var(--text)" }}>{p.value}</span>
                <Link
                  href={p.key === "custom" ? "/contact" : "/pricing"}
                  className="btn-pop"
                  style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 14, fontWeight: 500, color: "var(--accent2)", border: "1px solid var(--border)", borderRadius: 10, padding: "10px 14px" }}
                >
                  {p.cta} →
                </Link>
              </div>
