...
import {
  PencilIcon, MicIcon, SlidersIcon, BookmarkIcon,
...
const PERSONALISATION: [React.ReactNode, string, string, string][] = [
  [<PencilIcon key="a" size={20} />, "var(--purple)", "Your style", "Choose how each edition should feel."],
  [<MicIcon key="b" size={20} />, "var(--orange)", "Your voice", "Select the voice that fits your publication."],
  [<SlidersIcon key="c" size={20} />, "var(--accent2)", "Your preferences", "Keep recurring choices consistent across editions."],
  [<BookmarkIcon key="d" size={20} />, "var(--purple)", "Pronunciations", "Make sure names, brands, and technical terms sound right."],
];

const DECISION_GUIDE: [string, string][] = [
  ["Want to try one edition", "One-Time"],
  ["Publish occasionally", "Monthly"],
  ["Publish consistently", "Annual"],
  ["Have specific requirements", "Custom"],
];

const FAQ_DATA = [
  ["What exactly is included in a Brief?", "Every Brief includes the audio briefing, the full script, key insights, ready-to-share promotional copy, and (on eligible plans) saved voice and pronunciation preferences."],
  ...
  ["What happens after I purchase?", "You'll get access to your workspace, where you submit your newsletter link and choose your preferences for each Brief."],
  ["Can I change my plan later?", "Yes. Reach out to our team any time to move between plans as your publishing needs change."],
  ...
  ["Can I speak with someone before choosing?", "Of course. Use Check Custom Pricing for an estimate, or reach out on the Contact page to talk it through first."],
];
...
export function PricingClient() {
  ...
  const plans = [
    ...
    },
    {
      ...
      color: "var(--accent2)",
      name: "Monthly",
      desc: "For regular publishing.",
      price: fmt(pricing.symbol, pricing.monthly),
      per: "/ month",
      note: "Best for consistent publishing.",
      cta: "Start Monthly",
      featured: false,
    },
    {
      ...
      color: "var(--orange)",
      name: "Annual",
      desc: "For regular publishing at the best value.",
      price: fmt(pricing.symbol, pricing.annual),
      per: "/ year",
      note: `${fmt(pricing.symbol, monthlyEquivalent)}/month when billed annually`,
      cta: "Choose Annual",
      featured: true,
      badge: "Best Value",
    },
  ...
  return (
    <>
      {/* 01 — Hero */}
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "76px 40px 64px", textAlign: "center" }}>
        <Reveal>
      ...
      <section style={{ background: "var(--band)", color: "#fff" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "72px 40px", textAlign: "center" }}>
          <Reveal>
            <h2 style={{ fontFamily: "var(--font-heading)", fontWeight: 900, fontSize: 34, letterSpacing: "-.03em", margin: "0 0 16px" }}>
              More than an <span style={{ color: "var(--accent2)" }}>audio version.</span>
            </h2>
            <p style={{ fontSize: 17, opacity: 0.88, margin: 0, maxWidth: "36em", marginLeft: "auto", marginRight: "auto" }}>
              Every edition becomes a complete Brief — with audio, publishing assets, saved preferences, and
              everything organised for your next issue.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "80px 40px" }}>
        <div style={{ textAlign: "center", maxWidth: "36em", margin: "0 auto 40px" }}>
          <h2 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 32, letterSpacing: "-.025em", margin: 0 }}>
            Make every Brief sound like your publication.
          </h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
          {PERSONALISATION.map(([icon, color, title, body], i) => (
            <Reveal key={title} delay={i * 70}>
              <div className="hover-pop" style={{ borderTop: `3px solid ${color}`, borderLeft: "1px solid var(--border)", borderRight: "1px solid var(--border)", borderBottom: "1px solid var(--border)", borderRadius: 16, background: "var(--card)", padding: "24px 20px", display: "flex", flexDirection: "column", gap: 12, minHeight: 160 }}>
                <span className="icon-badge icon-glow" style={{ width: 42, height: 42, borderRadius: 12, background: `color-mix(in srgb, ${color} 14%, var(--card))`, color }}>{icon}</span>
                <span style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 17 }}>{title}</span>
                <span style={{ fontSize: 13.8, color: "var(--text2)" }}>{body}</span>
              </div>
            </Reveal>
          ))}
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, alignItems: "stretch" }}>
            {plans.map((p, i) => (
              <Reveal key={p.key} delay={i * 80}>
                ...
                >
                  ...
                  {p.note && <div style={{ fontSize: 12.5, color: p.featured ? "var(--accent2)" : "var(--accent2)", fontWeight: 500 }}>{p.note}</div>}
                  <Link
            ...
            <Reveal delay={240}>
              <div style={{ position: "relative", background: "var(--card)", border: "1px dashed var(--border)", borderRadius: 16, padding: "26px 22px", display: "flex", flexDirection: "column", gap: 10, minHeight: 240 }}>
                ...
                <span style={{ fontSize: 14, color: "var(--text2)" }}>A tailored setup for your publication.</span>
                <span style={{ fontSize: 12.5, color: "var(--text3)", flex: 1 }}>For publishers with specific requirements, workflows, or higher volume.</span>
                <CustomPlanFlow />
              </div>
            </Reveal>
          </div>
      ...
      {/* 06 — Compare everything included */}
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 40px" }}>
        <div style={{ textAlign: "center", maxWidth: "34em", margin: "0 auto 36px" }}>
          <h2 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 30, letterSpacing: "-.025em", margin: "0 0 8px" }}>Everything included, clearly compared.</h2>
          <p style={{ fontSize: 15, color: "var(--text2)", margin: 0 }}>See what&apos;s included with each way of publishing.</p>
        </div>
        <div style={{ border: "1px solid var(--border)", borderRadius: 18, background: "var(--card)", overflow: "hidden", boxShadow: "var(--shadow)", overflowX: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 1fr 1fr", minWidth: 640 }}>
            <div style={{ padding: "16px 20px" }} />
            {["One-Time", "Monthly", "Annual", "Custom"].map((h) => (
              <div key={h} style={{ padding: "16px 12px", textAlign: "center", fontSize: 11, letterSpacing: ".1em", textTransform: "uppercase", color: h === "Annual" ? "#fff" : "var(--text3)", background: h === "Annual" ? "var(--band)" : "transparent" }}>
                {h}
              </div>
            ))}
          </div>
          {[
            ["Newsletter briefings", "1", "4 / month", "48 / year", "By plan"],
            ["Style & voice selection", 1, 1, 1, 1],
            ["Optional script review", 1, 1, 1, 1],
            ["Custom voice creation", 0, 1, 1, "Available"],
            ["Saved preferences", 0, 1, 1, 1],
            ["Pronunciation guidance", 1, 1, 1, 1],
            ["Publishing assets", 1, 1, 1, 1],
            ["Personal workspace", 1, 1, 1, 1],
          ].map(([label, one, month, annual, custom], i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 1fr 1fr", borderTop: "1px solid var(--border2)", minWidth: 640 }}>
              <div style={{ padding: "13px 20px", fontSize: 13.8 }}>{label as string}</div>
              {[one, month, annual, custom].map((v, ci) => (
                <div key={ci} style={{ padding: "13px 12px", textAlign: "center", fontSize: 13.5, background: ci === 2 ? "var(--tint)" : "transparent" }}>
                  {v === 1 ? (
      ...
      <section style={{ background: "var(--bg2)", borderTop: "1px solid var(--border2)", borderBottom: "1px solid var(--border2)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 40px" }}>
          <div style={{ textAlign: "center", maxWidth: "32em", margin: "0 auto 36px" }}>
            <h2 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 32, letterSpacing: "-.025em", margin: 0 }}>Find a voice that fits. Or create one.</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div className="hover-pop" style={{ borderTop: "3px solid var(--accent2)", border: "1px solid var(--border)", borderRadius: 16, background: "var(--card)", padding: "28px 30px", display: "flex", flexDirection: "column", gap: 10 }}>
              <span className="icon-badge icon-glow" style={{ background: "var(--tint)", color: "var(--accent2)" }}><MicIcon size={20} /></span>
              <span style={{ fontFamily: "var(--font-heading)", fontWeight: 900, fontSize: 22 }}>180+ voices to explore</span>
              <span style={{ fontSize: 14.5, color: "var(--text2)" }}>Find a voice that fits your publication.</span>
            </div>
            <div className="hover-pop" style={{ borderTop: "3px solid var(--purple)", background: "var(--band)", color: "#fff", borderRadius: 16, padding: "28px 30px", display: "flex", flexDirection: "column", gap: 10 }}>
              <span className="icon-badge icon-glow" style={{ background: "rgba(255,255,255,.14)", color: "var(--purple)" }}><MicIcon size={20} /></span>
              <span style={{ fontFamily: "var(--font-heading)", fontWeight: 900, fontSize: 22 }}>Create your own voice</span>
              <span style={{ fontSize: 14.5, opacity: 0.85 }}>For a more distinctive publishing experience.</span>
            </div>
          </div>
        </div>
      </section>

      {/* 08 — Process teaser (compact) */}
      <section style={{ maxWidth: 1000, margin: "0 auto", padding: "72px 40px" }}>
        <div style={{ border: "1px solid var(--border)", borderRadius: 18, background: "var(--card)", padding: "32px 36px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 18, marginBottom: 4 }}>Simple from edition to delivery.</div>
            <div style={{ fontSize: 13.5, color: "var(--text2)" }}>Choose your plan → Add your edition → Review your Brief → Publish &amp; share</div>
          </div>
          <Link href="/flow" style={{ fontSize: 14, color: "var(--accent2)", fontWeight: 500, flex: "none" }}>See how it works →</Link>
        </div>
      </section>
      ...
      <section style={{ background: "var(--bg2)", borderTop: "1px solid var(--border2)", borderBottom: "1px solid var(--border2)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "72px 40px" }}>
          <h2 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 28, letterSpacing: "-.025em", margin: "0 0 24px", textAlign: "center" }}>Not sure which plan fits?</h2>
          <div style={{ border: "1px solid var(--border)", borderRadius: 14, background: "var(--card)", overflow: "hidden" }}>
            {DECISION_GUIDE.map(([q, a], i) => (
              <div key={q} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderTop: i === 0 ? "none" : "1px solid var(--border2)" }}>
                <span style={{ fontSize: 14.5, color: "var(--text2)" }}>{q}</span>
                <span style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 14.5, color: "var(--accent2)" }}>{a}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 10 — FAQ */}
      <section style={{ maxWidth: 1000, margin: "0 auto", padding: "80px 40px 56px", display: "grid", gridTemplateColumns: ".8fr 1.2fr", gap: 56, alignItems: "start" }}>
        <div>
